...
const order = computed(() => orderStore.getById(id.value))
...
<template>
  <div v-if="order" class="detail-wrap">
    ...
    <OrderFormDialog v-model="dialogVisible" mode="edit" :order-id="order.id" @saved="() => {}" />
  </div>

  <el-result v-else icon="warning" title="未找到订单" sub-title="请从订单列表进入或检查链接">
    <template #extra>
